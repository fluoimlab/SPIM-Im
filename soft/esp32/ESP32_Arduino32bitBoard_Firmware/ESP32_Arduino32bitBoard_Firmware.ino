...
// Set the output channnels, these are the ones for most handy for ESP32-32D with pin expansion board
const int inPin_ = 13;
const int ch1 = 15; 
const int ch2 = 2;
const int ch3 = 4;
...
// Helper to map legacy channel index to pin
int getPinForChannel(int channel) {
  switch (channel) {
    case 2: return ch3; // pwmCh3 was channel 2
    case 3: return ch4; // pwmCh4 was channel 3
    case 4: return ch5; // pwmCh5 was channel 4
    case 5: return ch6; // pwmCh6 was channel 5
    case 6: return ch7; // pwmCh7 was channel 6
    case 7: return ch8; // pwmCh8 was channel 7
    default: return -1;
  }
}
...
void setup() {
  ...
  // Setting up pwm
  // ESP32 Core v3.0+ migration: ledcAttach(pin, freq, resolution)
  ledcAttach(ch3, PWMfreq, PWMresolution);
  ledcAttach(ch4, PWMfreq, PWMresolution);
  ledcAttach(ch5, PWMfreq, PWMresolution);
  ledcAttach(ch6, PWMfreq, PWMresolution);
  ledcAttach(ch7, PWMfreq, PWMresolution);
  ledcAttach(ch8, PWMfreq, PWMresolution);
          ...
          if (waitForSerial(timeOut_))
          {
            ...
            if (waitForSerial(timeOut_)) {
              ...
              if (channel == 0) {
                ...
                // ESP32 Core v3.0+ migration: ledcWrite takes pin, not channel
                int pin = getPinForChannel(channel);
                if (pin != -1) {
                    ledcWrite(pin, msblsb[channel]);
                }
              }
              Serial.write( byte(3));
        ...
        if (waitForSerial(timeOut_)) {
          ...
          if ( (patternNumber >= 0) && (patternNumber < SEQUENCELENGTH) ) {
            if (waitForSerial(timeOut_)) {
              ...
              // triggerPattern_[patternNumber] = triggerPattern_[patternNumber] & B00111111; Removed for additions extra channels
              Serial.write( byte(5));
...
byte writeZeros()
{
  dacWrite(25, 0);
  dacWrite(26, 0);
  // ESP32 Core v3.0+ migration: ledcWrite takes pin
  ledcWrite(ch3, 0);
  ledcWrite(ch4, 0);
  ledcWrite(ch5, 0);
  ledcWrite(ch6, 0);
...
void writePattern(byte pattern_)
{
  ...
  for (int i = 2; i <= 7; i++)
  {
    ...
    {
      ledcWrite(getPinForChannel(i), 0);
    }
    else {
      ledcWrite(getPinForChannel(i), msblsb[i]);
    }
  }
  /*
